...
(function () {
  ...
  // backend (misma Apps Script del RSVP, dos pestañas nuevas en el
  // Sheet: Regalos y Aportes). Si la API no responde (todavía no
  // desplegado, o mientras se prueba en local sin server/dev_api.py),
  // esta sección completa se queda oculta — no rompe nada mientras
  // tanto, y el resto de "Mesa de regalos" (Yape/transferencia general,
  // lista externa) sigue funcionando igual que antes. —
  function initGiftRegistry(guestPromise) {
    ...
    var altHead = document.querySelector("#gift-alt-head");
    if (!grid || !panel) return;

    var url = W.rsvp && W.rsvp.apiUrl;
    if (!url) return; // sin backend no hay de dónde traer la lista
    ...
    function renderGrid() {
      ...
      grid.hidden = regalos.length === 0;
      if (altHead) altHead.hidden = regalos.length === 0;
    }
    ...
    function cargarRegalos() {
      fetch(url + "?tipo=regalos", { cache: "no-store" })
        ...
        .then(function (body) {
          if (!body || !Array.isArray(body.regalos)) return; // backend viejo o sin esta ruta todavía
          regalos = body.regalos;
          renderGrid();
        })
        .catch(function () { /* sin lista, la sección se queda oculta */ });
    }
